...
class LoonUserView(LoonBaseView):
    post_schema = Schema({
        ...
        'email': And(str, lambda n: n != '', error='name is needed'),
    ...
    def post(self, request, *args, **kwargs):
        ...
        mobile = request_data_dict.get('mobile')
        ...
        flag, result = account_base_service_ins.add_user(username, name, email, mobile, dept_ids, is_active, type_id, creator, password)
        if flag is False:
...
class LoonUserDetailView(LoonBaseView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        is_active = request_data_dict.get('is_active')
        flag, result = account_base_service_ins.edit_user(user_id, username, name, email, mobile, dept_ids, is_active,
                                                          type_id)
        if flag is not False:
            code, msg, data = 0, '', {}
        else:
...
class LoonDeptView(LoonBaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        owner = request_data_dict.get('owner')
        approver = request_data_dict.get('approver')

        label = request_data_dict.get('label')
        creator = request.user.username
        flag, result = account_base_service_ins.add_dept(name, parent_dept_id, owner, approver, label, creator)
        if flag is False:
            return api_response(-1, result, {})
        return api_response(0, result, {})
...
class LoonDeptDetailView(LoonBaseView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        flag, result = account_base_service_ins.update_dept(dept_id,name, parent_dept_id, owner,
                                                            approver, label)
        if flag is False:
            return api_response(-1, result, {})
        return api_response(0, '', {})
