...
from flow.account.models import AppToken, LoonUserRole, LoonUserDept
from flow.service.base_service import BaseService
from flow.service.common.constant_service import constant_service_ins
from flow.service.common.log_service import auto_log
...
class AccountBaseService(BaseService):
    ...
    def get_user_by_username(cls, username: str)->tuple:
        ...
        :return:
        """
        result = Users.objects.filter(username=username, is_deleted=0).first()
        if result:
            return True, result
        else:
    ...
    def get_user_list_by_usernames(cls, usernames: list)->tuple:
        ...
        :return:
        """
        result = Users.objects.filter(username__in=usernames, is_deleted=0).all()
        if result:
            return True, result
        else:
    ...
    def get_user_by_user_id(cls, user_id: int)->tuple:
        ...
        :return:
        """
        result = Users.objects.filter(id=user_id, is_deleted=0).first()
        if result:
            return True, result
        else:
    ...
    def get_user_name_list_by_id_list(cls, user_id_list: list)->tuple:
        ...
        :return:
        """
        user_queryset = Users.objects.filter(id__in=user_id_list, is_deleted=0).all()
    ...
    def get_user_role_id_list(cls, username: str)->tuple:
        ...
        :return:
        """
        user_obj = Users.objects.filter(username=username, is_deleted=0).first()
        if not user_obj:
    ...
    def get_role_user_info_by_role_id(cls, role_id: int, search_value: str='', page: int=1, per_page: int =10)->tuple:
        ...
        user_info_queryset = Users.objects.filter(query_params).all()
    ...
    def get_user_up_dept_id_list(cls, username: str)->tuple:
        ...
        def iter_dept(dept_id):
            dept_obj = Dept.objects.filter(id=dept_id, is_deleted=0).first()
            if dept_obj:
    ...
    def get_user_dept_approver(cls, username: str, dept_id: int=0)->tuple:
        """
        get user's department approver， Preferential access to the approver, without taking tl（team owner）
        :param username:
        ...
        if dept_id:
            if LoonUserDept.objects.filter(user_id=user_obj.id, dept_id=dept_id, is_deleted=0).first():
                ...
                if loon_dept_obj.approver:
                    ...
                    return True, loon_dept_obj.owner
            else:
            ...
            for user_dept in user_dept_queryset:
                if user_dept.dept.approver:
                    ...
                    approver_list.append(user_dept.dept.owner)
    ...
    def get_dept_sub_dept_id_list(cls, dept_id: int)->tuple:
        ...
        def iter_dept_id_list(new_dept_id):
            new_dept_obj = Dept.objects.filter(id=new_dept_id, is_deleted=0).first()
            if new_dept_obj:
                sub_dept_queryset = Dept.objects.filter(parent=new_dept_obj.id, is_deleted=0).all()
    ...
    def get_dept_username_list(cls, dept_id: object)->tuple:
        ...
        user_queryset = Users.objects.filter(id__in=user_id_list).all()
    ...
    def get_role_username_list(cls, role_id: int)->tuple:
        ...
        if not user_id_list:
            return True, []
        username_queryset = Users.objects.filter(id__in=user_id_list).all()
        username_list = []
    ...
    def app_workflow_permission_list(cls, app_name: str)->tuple:
        ...
        if app_name == 'loonflow':
            ...
            from flow.workflow.models import Workflow
    ...
    def app_workflow_permission_check(cls, app_name: str, workflow_id: int)->tuple:
        ...
        from flow.service.workflow.workflow_permission_service import workflow_permission_service_ins
    ...
    def app_ticket_permission_check(cls, app_name: str, ticket_id: int)-> tuple:
        ...
        :return:
        """
        from flow.service.ticket.ticket_base_service import ticket_base_service_ins
    ...
    def get_user_list(cls, search_value: str, page: int=1, per_page: int=10, simple=False)->tuple:
        ...
        :param search_value: support user's username, and user's name. fuzzy query
        :param page:
        :param per_page:
        ...
        if search_value:
            query_params &= Q(username__contains=search_value) | Q(alias__contains=search_value)
        user_objects = Users.objects.filter(query_params)
        ...
        for user_result_object in user_result_object_list:
            ...
            if simple:
                ...
                user_result_format_dict.pop('mobile')
                user_result_format_dict.pop('type_id')
                user_result_format_dict.pop('create_datetime')
                user_result_format_dict.pop('update_datetime')
                user_result_format_dict.pop('is_deleted')
                dept = user_result_format_dict['dept']
                if dept is not None:
                    user_result_format_dict['dept'] = dept.id
    ...
    @classmethod
    @auto_log
    def add_user(cls, username: str, name: str, email: str, mobile: str, dept_ids: str, is_active: int,
        ...
        user_obj = Users(username=username, name=name, email=email, mobile=mobile,
    ...
    @classmethod
    @auto_log
    def edit_user(cls, user_id: int, username: str, name: str, email: str, mobile: str, dept_ids: str, is_active: int,
                  type_id: int)-> tuple:
        """
        edit user
        :param user_id:
        ...
        :return:
        """
        user_obj = Users.objects.filter(id=user_id, is_deleted=0)
        user_obj.update(username=username, name=name, email=email, mobile=mobile, is_active=is_active,
                        type_id=type_id)
        # todo 更新部门信息
    ...
    def update_role(cls, role_id: int, name: str, description: str, label: str)-> tuple:
        ...
        :return:
        """
        role_queryset = Role.objects.filter(id=role_id, is_deleted=0)
    ...
    def get_dept_list(cls, search_value: str, page: int=1, per_page: int=10, simple=False)->tuple:
        ...
        for dept_result_object in dept_result_object_list:
            ...
            result_dict['creator'] = result_dict['creator'].id
            result_dict.pop('creator_info')
            result_dict.pop('parent_dept_info')
            result_dict.pop('approver_info')
            result_dict.pop('leader_info')
            if result_dict['parent'] is not None:
                result_dict['parent'] = result_dict['parent'].id
    ...
    @classmethod
    @auto_log
    def add_dept(cls, name: str, parent_dept_id: int, owner: str, approver: str, label: str, creator: str)->tuple:
        """
        add department
        新增部门
        :param name:
        ...
        :return:
        """
        dept_obj = Dept(name=name, parent_dept_id=parent_dept_id, owner=owner, approver=approver, label=label,
                            creator=creator)
        dept_obj.save()
    ...
    @classmethod
    @auto_log
    def update_dept(cls, dept_id: int, name: str, parent_dept_id: int, owner: str, approver: str, label: str)->tuple:
        """
        ...
        :return:
        """
        dept_queryset = Dept.objects.filter(id=dept_id, is_deleted=0)
        if not dept_queryset:
            return False, 'dept is not existed or has been deleted'
        dept_queryset.update(name=name, parent_dept_id=parent_dept_id, owner=owner, approver=approver, label=label)
        return True, ''
    ...
    def add_token_record(cls, app_name: str, ticket_sn_prefix: str, workflow_ids: str, username: str)-> tuple:
        ...
        from flow.workflow.models import WorkflowUserPermission
        permission_sql_list = []
        if workflow_ids:
    ...
    def get_user_jwt(cls, username: str)->tuple:
        ...
        dept = user_info['dept']
        if dept is not None:
            user_info['dept'] = user_info['dept'].id
        jwt_info = jwt.encode(
                              {
