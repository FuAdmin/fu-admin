...
from flow.workflow.models import CustomField
from flow.ticket.models import TicketRecord, TicketCustomField, TicketFlowLog, TicketUser
from flow.service.redis_pool import POOL
from flow.service.base_service import BaseService
from flow.service.common.log_service import auto_log
from flow.service.common.common_service import common_service_ins
...
from flow.service.workflow.workflow_base_service import workflow_base_service_ins
from flow.service.workflow.workflow_state_service import workflow_state_service_ins
from flow.service.workflow.workflow_transition_service import workflow_transition_service_ins
from flow.service.workflow.workflow_custom_field_service import workflow_custom_field_service_ins
...
class TicketBaseService(BaseService):
    ...
    def get_ticket_list(cls, sn: str='', title: str='', username: str='', create_start: str='', create_end: str='',
        ...
        for ticket_result_object in ticket_result_object_list:
            ...
            if flag:
                ...
                creator_info = dict(username=creator_obj.username, name=creator_obj.name,
                                    is_active=creator_obj.is_active, email=creator_obj.email, mobile=creator_obj.mobile,
                                    dept_info=dept_dict_info)
            else:
                creator_info = dict(username=ticket_result_object.creator, name='', is_active=False, email='',
                                    mobile='', dept_info={})
    ...
    def new_ticket(cls, request_data_dict: dict, app_name: str='')->tuple:
        ...
        if not add_ticket_flow_log_result:
            return False, msg
        # 通知消息workflow
        from flow.tasks import send_ticket_notice
        ...
        if destination_participant_type_id == constant_service_ins.PARTICIPANT_TYPE_ROBOT:
            from flow.tasks import run_flow_task  # 放在文件开头会存在循环引用
        ...
        if destination_participant_type_id == constant_service_ins.PARTICIPANT_TYPE_HOOK:
            from flow.tasks import flow_hook_task  # 放在文件开头会存在循环引用
    ...
    def get_ticket_detail(cls, ticket_id: int, username: str)-> tuple:
        ...
        if flag:
            ...
            creator_info = dict(username=creator_obj.username, name=creator_obj.name,
                                ...
                                mobile=creator_obj.mobile, dept_info=dept_dict_info)
        else:
            creator_info = dict(username=ticket_obj.creator, name='', is_active=False, email='', mobile='',
                                dept_info={})
        # 当前状态信息
    ...
    def get_ticket_format_participant_info(cls, ticket_id: int)->tuple:
        ...
        if participant_type_id == constant_service_ins.PARTICIPANT_TYPE_PERSONAL:
            ...
            if flag:
                participant_alias = participant_user_obj.name
            else:
            ...
            participant_map = {"name": set(), "name": set()}
            ...
            if flag:
                ...
                participant_map["name"].update(
                    set(participant_name_list) - participant_map["name"]
                )

            participant_alias = ','.join(participant_map["name"])
            ...
            # 脚本类型参数与人是脚本记录的id
            from flow.workflow.models import WorkflowScript
        ...
        if json.loads(ticket_obj.multi_all_person):
            ...
            for key, value in multi_all_person_dict.items():
                ...
                if not flag:
                    ...
                    participant_alias0 = participant_user_obj.name
                if value:
    ...
    def get_ticket_flow_log(cls, ticket_id: int, username: str, per_page: int=10, page: int=1, ticket_data=0, desc=1)->tuple:
        ...
        for ticket_flow_log in ticket_result_paginator.object_list:
            ...
            if ticket_flow_log.participant_type_id == constant_service_ins.PARTICIPANT_TYPE_PERSONAL:
                ...
                if flag:
                    participant_info.update(participant_alias=participant_query_obj.name,
                                            participant_email=participant_query_obj.email,
                                            participant_phone=participant_query_obj.mobile
                                            )
        ...
        if destination_transition_queryset:
            for destination_transition in destination_transition_queryset:
                if destination_transition.timer:
                    from flow.tasks import timer_transition
    ...
    def retry_ticket_script(cls, ticket_id: int, username: str)->tuple:
        ...
        if ticket_obj.participant_type_id == constant_service_ins.PARTICIPANT_TYPE_ROBOT:
            ...
            ticket_obj.save()
            from flow.tasks import run_flow_task  # 放在文件开头会存在循环引用问题
            run_flow_task.apply_async(args=[ticket_id, ticket_obj.participant, ticket_obj.state_id,
            ...
            ticket_obj.script_run_last_result = True
            ticket_obj.save()
            from flow.tasks import flow_hook_task
            flow_hook_task.apply_async(args=[ticket_id], queue='loonflow')
            return True, ''
        else:
    ...
    def get_ticket_participant_info(cls, ticket_id: int)->tuple:
        ...
        if participant_username_list:
            participant_queryset = Users.objects.filter(username__in=participant_username_list, is_deleted=0)
            for participant_0 in participant_queryset:
                participant_info_list.append(dict(username=participant_0.username, name=participant_0.name,
                                                  mobile=participant_0.mobile, email=participant_0.email))
